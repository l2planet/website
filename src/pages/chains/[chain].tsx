...
const Chain: NextPage = () => {
    ...
    return(
        <>
            ...
            <SectionChainIntro>
                    <Img src={chain?.icon || SPARE_ICON} img={ImgChain} alt={`${chain?.name || 'Chain'} Logo`}/>
                <DivMeta>
                    <HChain>{chain?.name || 'Chain'}</HChain>
                    <PChainDescription>{chain?.description || 'Lorem ipsum dolor sit amet, et lorem ipsum dolor sit amet.'}</PChainDescription>
                </DivMeta>
            </SectionChainIntro>
            <SectionChainL2s>
                <Col4Select>
                    <HSelect>Layer 2 Category:</HSelect>
                    <Select onSelect={(category) => setCategory(category)} options={chain?.l2Categories || []}/>
                </Col4Select>
                <Grid4ChainPage>
                    {chain?.layer2s.at(0) ?
                        <>
                            {category ?
                            chain?.layer2s.filter(l2 => l2.categories.includes(category)).map(l2 => <CardLayer2 key={l2.id} {...l2}/>) :
