...
import { isReadyToBeDeletedAt, onKeyDownHandler, resizeTextareaHeight, getCaretPos, tool, directions, getValidImage } from "./functions"
    ...
    return (
        ...
        <>
            ...
            <BlockHolder>
                ...
                <EditableBlock
                    ...
                    onChange={e => {
                        ...
                        if(block.is('P') || block.is('I')  || block.is('W') || block.is('V')) {
                            switch(true) {
                                case new TwitterTweet(e.target.value).getId() !== null:
                                    blocks[i] = block.as('W')
                                    break
                                case new YoutubeVideo(e.target.value).getId() !== null:
                                    blocks[i] = block.as('V')
                                    break
                ...
                <ButtonSend onClick={() => {onSubmit(filterBlocks())}}>Send</ButtonSend>
            </BlockHolder>
        </>
...
const Tool = ({ isDivision, changeType }: { isDivision: MutableRefObject<boolean>, changeType(newType: EditorBlockType): void }) => (
    ...
    <WTool id='tool' onClick={() => {tool.hide(); isDivision.current = false}}>
        <Button onClick={() => changeType('H1')} className='rounded-t-xl'>Heading 1</Button>
